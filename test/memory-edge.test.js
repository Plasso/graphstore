...
test('getLimitOffset returns edges added in order', async () => {
  const edgeName = 'test_edge';
  const ed = new MemoryEdge(edgeName);
  ...
  const page = await ed.getLimitOffset('leftId', { offset: 1, limit: 2 });
...
test('getLimitOffset returns hasNextPage: true if more edges', async () => {
  const edgeName = 'test_edge';
  const ed = new MemoryEdge(edgeName);

  await ed.create('leftId', 'rightId1');
  await ed.create('leftId', 'rightId2');
  const page = await ed.getLimitOffset('leftId', { offset: 0, limit: 1 });

  expect(page.hasNextPage).toBe(true);
});

test('getLimitOffset returns hasNextPage: false if no more edges', async () => {
  const edgeName = 'test_edge';
  const ed = new MemoryEdge(edgeName);

  await ed.create('leftId', 'rightId1');
  await ed.create('leftId', 'rightId2');
  const page = await ed.getLimitOffset('leftId', { offset: 0, limit: 2 });
